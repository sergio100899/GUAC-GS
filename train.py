...
def training(
    ...
    # Pre-calculation of all GT depths to avoid doing it in the training loop
    print("Pre-calculating GT depth maps for training cameras...")
    with torch.no_grad():
        for view in tqdm(scene.getTrainCameras(), desc="Depth Pre-calculation"):
            if not hasattr(view, "gt_depth"):
                ...
                # Store it on CPU to save VRAM, will be moved to GPU in the loop
                view.gt_depth = gt_depth.cpu()
        release_depth_model()  # Release the model immediately after pre-calculation
        print("GT depth maps pre-calculated and depth model released from GPU.")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        L_edge, var_edge = edge_loss(image, gt_image)

        L_depth, var_depth = compute_depth_loss_from_normalized_gradients(
            depth_map.squeeze(),
            viewpoint_cam.gt_depth.cuda(),
            gt_image,
        )
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    l1_test += l1_loss(image, gt_image)[0].mean().double()
